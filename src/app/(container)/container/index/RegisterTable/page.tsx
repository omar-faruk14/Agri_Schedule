...
const ContainerRegisterTable = () => {
    ...
    const fetchData = async () => {
      ...
      try {
        const response = await fetch(
          `/api/container/register/DynamicRegister?page=${page}&limit=${limit}&typeCode=container&search=${encodeURIComponent(
            debouncedSearch
          )}`
        );
        if (!response.ok) {
  ...
  const handleDelete = async (recordId: string) => {
    const result = await Swal.fire({
      title: "削除してもよろしいですか？",
      html: "この操作は元に戻せません！<br>この画面から削除しても、「コンテナ登録」セクションからのみ削除されます。すでに使用中のステータスになっているコンテナには影響しません。",
      icon: "warning",
          ...
          <div className="card-body">
            <div className="container-fluid">
              <div className="row mb-2">
                <div className="col">
                  <h2 className={`${styles.h2_map}`}>コンテナを登録一覧</h2>
                </div>
              </div>
            </div>
            <input
